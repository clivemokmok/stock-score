...
def check_minervini(hist, spy_hist):
    ...
    rs_last = rs_series.dropna().iloc[-1]
rs_ma_last = rs_ma50.dropna().iloc[-1]
if hasattr(rs_last, 'iloc'):
    rs_last = float(rs_last.iloc[0])
if hasattr(rs_ma_last, 'iloc'):
    rs_ma_last = float(rs_ma_last.iloc[0])
rs_ok = float(rs_last) > float(rs_ma_last)
rs_vs_ma = (float(rs_last) / float(rs_ma_last) - 1) * 100
