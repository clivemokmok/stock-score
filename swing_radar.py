...
lc = to_float(close.iloc[-1])
e10 = to_float(ema10.iloc[-1])
e20 = to_float(ema20.iloc[-1])
e50 = to_float(ema50.iloc[-1])
e150 = to_float(ema150.iloc[-1])
e200 = to_float(ema200.iloc[-1])
trend_ok = (lc > e150) and (lc > e200) and (e150 > e200)
momentum_ok = (e10 > e20) and (e20 > e50)
high_52w = to_float(close.tail(252).max())
...
rs_last = to_float(rs_series.dropna().iloc[-1])
rs_ma_last = to_float(rs_ma50.dropna().iloc[-1])
...
ema_val = to_float(ema_series.iloc[-1])
recent_lows = [to_float(low.iloc[-1]), to_float(low.iloc[-2])]
touched = any(abs(l - ema_val) / ema_val <= EMA_TOUCH_PCT for l in recent_lows)
above = to_float(close.iloc[-1]) > ema_val
low_vol = to_float(volume.iloc[-1]) < to_float(avg_vol_50.iloc[-1])
if touched and above and low_vol:
setups.append({‘type’: ‘Setup1’, ‘ema_name’: ema_name, ‘ema_val’: round(ema_val,2), ‘vol_ratio’: round(to_float(volume.iloc[-1])/to_float(avg_vol_50.iloc[-1]),2), ‘vcp’: False})
if len(high) >= 5:
h5 = to_float(high.iloc[-5:].max())
l5 = to_float(low.iloc[-5:].min())
rng5 = (h5 - l5) / l5
if rng5 < TIGHT_RANGE_PCT:
vcp_flag = False
if len(high) >= VCP_LOOKBACK:
mid = VCP_LOOKBACK // 2
l_f = to_float(low.iloc[-VCP_LOOKBACK:-mid].min())
l_l = to_float(low.iloc[-mid:].min())
rf = (to_float(high.iloc[-VCP_LOOKBACK:-mid].max()) - l_f) / l_f if l_f > 0 else 0
rl = (to_float(high.iloc[-mid:].max()) - l_l) / l_l if l_l > 0 else 0
...
continue
if to_float(hist[‘Close’].iloc[-1]) < MIN_PRICE:
continue
if to_float(hist[‘Volume’].tail(50).mean()) < MIN_AVG_VOL_50:
continue
